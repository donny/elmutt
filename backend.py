...
def process_message(message):
    ...
    if message['REQ'] == 'REFRESH':
        ...
        card_list.save()
        result = {'RESP': 'RESP_NEWLIST', 'IDENTIFIER': card_list.identifier, 'TEXT': card_list.text}
        ...
        card_list.save()
        result = {'RESP': 'RESP_RENAMELIST', 'IDENTIFIER': card_list.identifier, 'TEXT': card_list.text}
        ...
        card_list.save()
        result = {'RESP': 'RESP_NEWCARD', 'IDENTIFIER': card.identifier, 'LISTIDENTIFIER': card_list.identifier, 'TEXT': card.text}
    elif message['REQ'] == 'RENAMECARD':
        card = Card.objects.filter(identifier=message['IDENTIFIER']).first()
        card.text = message['TEXT']
        card.save()
        result = {'RESP': 'RESP_RENAMECARD', 'IDENTIFIER': card.identifier, 'LISTIDENTIFIER': message['LISTIDENTIFIER'], 'TEXT': card.text}
    elif message['REQ'] == 'UPVOTECARD':
        card = Card.objects.filter(identifier=message['IDENTIFIER']).first()
        card.incr('counter')
        card.save()
        result = {'RESP': 'RESP_UPVOTECARD', 'IDENTIFIER': card.identifier, 'LISTIDENTIFIER': message['LISTIDENTIFIER'], 'COUNTER': card.counter}
